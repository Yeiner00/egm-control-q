-- Corrige alias detectados despues de la primera limpieza y vuelve a compactar
-- personas historicas sin duplicar roles.
...
CREATE TEMP TABLE report_people_alias_cleanup_expanded ON COMMIT DROP AS
SELECT
  p.id AS old_id,
  p.reporte_id,
  p.tipo_reporte,
...
CREATE TEMP TABLE report_people_alias_cleanup_people ON COMMIT DROP AS
SELECT
...
FROM report_people_alias_cleanup_expanded
GROUP BY reporte_id, tipo_reporte, nombre_normalizado;

CREATE TEMP TABLE report_people_alias_cleanup_roles ON COMMIT DROP AS
SELECT
  cp.id AS reporte_persona_id,
  btrim(r.rol) AS rol,
  min(r.created_at) AS created_at
FROM report_people_alias_cleanup_expanded e
JOIN public.reporte_persona_roles r
  ON r.reporte_persona_id = e.old_id
JOIN report_people_alias_cleanup_people cp
...
FROM report_people_alias_cleanup_people;
...
FROM report_people_alias_cleanup_roles;

CREATE TEMP TABLE legacy_report_people_alias_cleanup ON COMMIT DROP AS
SELECT
  gen_random_uuid() AS id,
  p.reporte_id,
  p.tipo_reporte,
...
FROM legacy_report_people_alias_cleanup;
