-- Normaliza personas historicas de reportes y evita duplicados futuros.
-- Reescribe reporte_personas/reporte_persona_roles conservando reporte, tipo,
-- roles y cedulas cuando el registro original representa una sola persona.

CREATE OR REPLACE FUNCTION public.normalize_report_person_key(raw text)
RETURNS text
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
BEGIN
  RETURN CASE
    ...
    WHEN key IN ('randall mena villavicencio') THEN 'Randall Mena Villavicencio'
    ...
    WHEN key IN ('obed vasquez chaves') THEN 'Obed Vasquez Chaves'
...
CREATE TEMP TABLE report_people_cleanup_expanded ON COMMIT DROP AS
SELECT
  p.id AS old_id,
  p.reporte_id,
  p.tipo_reporte,
...
CREATE TEMP TABLE report_people_cleanup_people ON COMMIT DROP AS
SELECT
...
FROM report_people_cleanup_expanded
GROUP BY reporte_id, tipo_reporte, nombre_normalizado;

CREATE TEMP TABLE report_people_cleanup_roles ON COMMIT DROP AS
SELECT
  cp.id AS reporte_persona_id,
  btrim(r.rol) AS rol,
  min(r.created_at) AS created_at
FROM report_people_cleanup_expanded e
JOIN public.reporte_persona_roles r
  ON r.reporte_persona_id = e.old_id
JOIN report_people_cleanup_people cp
...
SELECT
  ...
  cedula,
  created_at
FROM report_people_cleanup_people;
...
SELECT
  ...
  rol,
  created_at
FROM report_people_cleanup_roles;

CREATE TEMP TABLE legacy_report_people_cleanup ON COMMIT DROP AS
SELECT
  gen_random_uuid() AS id,
  p.reporte_id,
  p.tipo_reporte,
...
SELECT
  ...
  rol,
  created_at
FROM legacy_report_people_cleanup;

DROP INDEX IF EXISTS public.reporte_personas_report_name_uidx;
CREATE UNIQUE INDEX reporte_personas_report_name_uidx
  ON public.reporte_personas (reporte_id, tipo_reporte, nombre_normalizado);

DROP INDEX IF EXISTS public.reporte_persona_roles_person_role_uidx;
CREATE UNIQUE INDEX reporte_persona_roles_person_role_uidx
  ON public.reporte_persona_roles (reporte_persona_id, rol);
